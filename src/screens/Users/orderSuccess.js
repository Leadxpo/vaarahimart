...
const OrderSuccess = ({ navigation, route }) => {
  ...
  const { order_id, orderDetails } = route.params;
  ...
  const successOrderBooking = async (paymentData, orderInfo) => {
    const payment_mode = paymentData.payment_method;
    ...
    try {
      ...
      await dispatch(deleteCart(userInfo.id));
      await dispatch(clearCart());
      if (orderDetails.paid_amt === paymentData.amount) {
  ...
  useEffect(() => {
    ...
    ApiClient.sendGetRequest(
      `https://vaarahimart.com/handleJuspayResponse`, order_id,
      {
  ...
  useEffect(() => {
    console.log("Updated responseText:", responseText);
  }, [responseText]);
