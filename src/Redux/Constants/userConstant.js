
export const GET_USERS_REQUEST = "GET_USERS_REQUEST";
export const GET_USERS_SUCCESS = "GET_USERS_SUCCESS";
export const GET_USERS_FAILURE = "GET_USERS_FAILURE";

export const USER_DETAIL_REQUEST = "USER_DETAIL_REQUEST";
export const USER_DETAIL_SUCCESS = "USER_DETAIL_SUCCESS";
export const USER_DETAIL_FAIL = "USER_DETAIL_FAIL";
export const USER_DETAIL_DELETE = "USER_DETAIL_DELETE";

export const CREATE_USER_REQUEST = "CREATE_USER_REQUEST";
export const CREATE_USER_FAIL = "CREATE_USER_FAIL";
export const CREATE_USER_SUCCESS = "CREATE_USER_SUCCESS";

export const UPDATE_USER_REQUEST = "UPDATE_USER_REQUEST";
export const UPDATE_USER_FAIL = "UPDATE_USER_FAIL";
export const UPDATE_USER_SUCCESS = "UPDATE_USER_SUCCESS";

export const UPDATE_USERDP_REQUEST = "UPDATE_USERDP_REQUEST";
export const UPDATE_USERDP_FAIL = "UPDATE_USERDP_FAIL";
export const UPDATE_USERDP_SUCCESS = "UPDATE_USERDP_SUCCESS";

export const DELETE_USER_REQUEST = "DELETE_USER_REQUEST";
export const DELETE_USER_FAIL = "DELETE_USER_FAIL";
export const DELETE_USER_SUCCESS = "DELETE_USER_SUCCESS";

export const DELETE_ALL_USER_REQUEST = "DELETE_ALL_USER_REQUEST";
export const DELETE_ALL_USER_FAIL = "DELETE_ALL_USER_FAIL";
export const DELETE_ALL_USER_SUCCESS = "DELETE_ALL_USER_SUCCESS";


